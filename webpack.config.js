...
module.exports = () => {
    let config = {
        entry: {
            "prenotazioni.min": path.resolve(__dirname, "resources/index"),
        },
    };
    ...
    config.output.path = path.resolve(
        __dirname,
        "src/redturtle/prenotazioni/browser/static/dist"
    );

    config.plugins.push(
        mf_config({
            name: "redturtle-prenotazioni",
            filename: "redturtle-prenotazioni.min.js",
            remote_entry: config.entry["prenotazioni.min"],
            dependencies: {
